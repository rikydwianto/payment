...
<div class="row">
    ...
    <form id='form_bayar' method="post">
        ...
        <div class="col-md-6">
            ...
            <tr>
                <td>MARGIN/DENDA</td>
                <td><input type="number" name='margin' min='0' value='0' class='form-control'></td>
            </tr>
            <tr>
                <td>TANGGAL</td>
...
if(isset($_POST['tmb_bayar'])){
    ...
    if($q)
    {
        ...
        mysqli_query($con,"INSERT into `kas`(akun,keterangan,masuk,status,payment_method,tanggal_kas,id_usaha)
                        values('401','penerimaan pembayaran wifi an $nama_pel bulan $priode tahun $tahun','$total','debit','$metode','$tgl','$id_usaha');
        ");
        mysqli_query($con,"INSERT into `jurnal`(akun,keterangan,keluar,status,payment_method,tanggal_kas,id_usaha,refrensi,urutan)
                        values('401','penerimaan pembayaran wifi an $nama_pel bulan $priode tahun $tahun','$tagihan','kredit','$metode','$tgl','$id_usaha','pem-$id_usaha-$id_kas',2);
        ");
        mysqli_query($con,"INSERT into `jurnal`(akun,keterangan,masuk,status,payment_method,tanggal_kas,id_usaha,refrensi,urutan)
                        values('101','Penerimaan kas','$total','debit','$metode','$tgl','$id_usaha','pem-$id_usaha-$id_kas','1');
        ");
        if($margin!=0){

            mysqli_query($con,"INSERT into `jurnal`(akun,keterangan,keluar,status,payment_method,tanggal_kas,id_usaha,refrensi,urutan)
                            values('410','penerimaan margin wifi an $nama_pel bulan $priode tahun $tahun','$margin','kredit','$metode','$tgl','$id_usaha','pem-$id_usaha-$id_kas','3');
            ");
        }
